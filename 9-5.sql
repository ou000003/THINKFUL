...
--Write a query that returns the year, make, model, and a field called average_mpg that 
--calculates the average highway/city fuel consumption. (For example, if hwy is 24 and cty is 20, then average_mpg = (24 + 20)/ 2 = 22.)
SELECT year, 
       make, 
       model, 
...
SELECT year, 
       make, 
	   model, 
	   CONCAT(hwy, ' highway; ', cty, ' city.') AS mpg_info
FROM public.vehicles;
...
--For example, use SELECT DISTINCT(drive) FROM vehicles; to get the unique values that appear in that field, or use SELECT DISTINCT(fuel) FROM vehicles;.
SELECT *
FROM public.vehicles
...
--Write a query that returns the count of records where the “cyl” field is NULL. Provide the query as well as the answerSELECT COUNT(*)
SELECT COUNT(*)
FROM public.vehicles
WHERE cyl IS NULL;
--58

--Write a query that returns the count of all records before the year 2000 that got more than 20 mpg hwy and had greater than 3 liters displacement (“displ” field). 
