...
FROM ksprojects
GROUP BY main_category
ORDER BY round(AVG(backers),0) DESC;
...
-- Write a query that shows, for each main category, how many projects had zero backers for that category and the largest goal amount for that category 
...
SELECT category, 
       avg(usd_pledged/NULLIF(backers,0))
FROM ksprojects
GROUP BY category
...
-- Excluding Games and Technology records in the main_category field, return the total of all backers for successful projects by main_category 
